...
def _start_video_decoder(video_path: str):
...
def _read_exact(pipe, num_bytes: int):
...
def _decode_audio_to_array(video_path: str, sample_rate: int, num_channels: int):
...
def stream_video(video_path: str, source_name: str = "StreamNDI", dual: bool = False):
    width, height, fps, total_frames = _probe_video(video_path)
    fps_float = float(fps)
    ...
    audio_data = _decode_audio_to_array(video_path, audio_sample_rate, audio_channels)
    ...
    video_proc = _start_video_decoder(video_path)
    ...
    with sender_plain:
        ...
        try:
            while True:
                raw = _read_exact(video_proc.stdout, frame_bytes)
                if raw is None:
