...
OPTIMIZED_AUDIO="Videos/Prod/Ghost_Towns_8K_AAC_48k_Stereo.m4a"
ORIGINAL_VIDEO="Videos/Ghost_Towns_in_8K_GoPro_be_Hero.webm"
if [[ -z "${NDI_VIDEO_PATH:-}" ]]; then
    if [[ -f "$SCRIPT_DIR/../$OPTIMIZED_VIDEO" && -f "$SCRIPT_DIR/../$OPTIMIZED_AUDIO" ]]; then
        export NDI_VIDEO_PATH="$OPTIMIZED_VIDEO"
        export NDI_AUDIO_PATH="$OPTIMIZED_AUDIO"
    else
        export NDI_VIDEO_PATH="$ORIGINAL_VIDEO"
        print -u2 -- "[NDI] Ghost Town optimized video/audio pair not found; using original VP9/Opus source."
    fi
fi
...
export NDI_VIDEO_PREFETCH_FRAMES="${NDI_VIDEO_PREFETCH_FRAMES:-6}"
